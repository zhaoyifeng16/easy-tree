...
const _flat = (tree) => {
  ...
  return tree.reduce((result, item) => {
    ...
    return result.concat(
      rest,
      children && children.length > 0 ? _flat(children) : []
    );
  }, []);
};
...
const _filterNode = (tree, func) => {
  return tree.reduce((result, node) => {
    const nodeMatches = func(node);
    let filteredChildren = [];

    if (node.children && node.children.length > 0) {
      filteredChildren = _filterNode(node.children, func);
    }
...
export { _flat, _filterNode, _insertHelper };
